...
void tache4(double *s,int a[],int l,int n,int h[] )
    {
        ...
        int c=0;
        while (e+c<l)
        {scanf ("donner operation que voulez vous effectuez echange ou comparaion %s",operation)
        if ((operation == '<') || (operation == '>'))
            e++;
        else if (operation='echange')
        {scanf("donner indice %d",i);
        scanf("donner indice %d",j);
        c++;
        t=a[i];
        a[i]=a[j];
        a[j]=t;}
...
int main()
{
    ...
    switch (i)
    {
        ...
        case 4:
            tache4(&s,int a[],int l, int n ,int h[]);   //h tableau triee et a tableau non trie et l max d operation
